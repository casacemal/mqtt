...
const char thingName[] = "Termostat-kombi";

// -- Initial password to connect to the Thing, when it creates an own Access Point.
const char wifiInitialApPassword[] = "termostat";

#define STRING_LEN 128
#define NUMBER_LEN 32
// -- Configuration specific key. The value should be modified if config structure was changed.
#define CONFIG_VERSION "kombi1"
...
//IotWebConfNumberParameter floatParam = IotWebConfNumberParameter("Float param", "floatParam", sensor, NUMBER_LEN,  nullptr, "e.g. 23.4", "step='0.1'");
...
void setup()
{
  ...
  if (!validConfig)
  {
    ...
    mqttpubSetTopic[0] = '\0';
    mqttpubodaTopic[0] = '\0';
    mqttpubyatakTopic[0] = '\0';
    mqttpubcocukTopic[0] = '\0';
    mqttpubIPTopic[0] = '\0';

    mqttsubdataSet[0] = '\0';
    mqttsubdataYatak[0] = '\0';
    mqttsubdataCocuk[0] = '\0';
    mqttsubdataOda[0] = '\0';
  }

  // -- Set up required URL handlers on the web server.
  server.on("/", handleRoot);
  //server.on("/handleODA",HTTP_POST, handleODA);
  // server.on("/hanYATAK",HTTP_POST, handleYATAK);
  //server.on("/hanCOCUK",HTTP_POST, handleCOCUK);
  server.on("/config", [] { iotWebConf.handleConfig(); });
  server.onNotFound([]() {
    iotWebConf.handleNotFound();
  });

  ///devices/testThing/status
...
void loop()
{
  ...
  // -- Check for button push buton ile role değiştirir
  ...
  //  && ( ACTION_FEQ_LIMIT < now - lastAction))
  if ( ACTION_FEQ_LIMIT < now - lastAction)
  { Serial.println("burda 5");
    ...
    String strc;
    strc = String(webyatak);
    strc.toCharArray(c, 5);


    String strd;
    strd = String(webcocuk);
    strd.toCharArray(d, 5);

    String stre;
    strd = String(mqttset);
    strd.toCharArray(e, 5);

    String strg;
    strg = String(temp);
    strg.toCharArray(g, 5);
    
    // String strf;
    // strd = String(WiFi.localIP());
    // strd.toCharArray(f, 5);
    // f = WiFi.localIP().toInt();
    char message[16];
    sprintf(message, "%u.%u.%u.%u", WiFi.localIP()[0], WiFi.localIP()[1], WiFi.localIP()[2], WiFi.localIP()[3]);
    /* char e[5];
      String stre;
      stre=String(webset);//yok şimdilik
       stre.toCharArray(e,3);*/

    mqttClient.publish(mqttpubodaTopic, b, false, 1); //oda
    mqttClient.publish(mqttpubyatakTopic, c, false, 1); //yatak
    mqttClient.publish(mqttpubcocukTopic, d, false, 1); //cocuk
    mqttClient.publish(mqttpubSetTopic, e, false, 1);//set
    mqttClient.publish(mqttpubIPTopic, message, true, 1);//ıp
    mqttClient.publish(mqttpubdataGelen,g,false,1); //gelen
    // mqttClient.publish("/mqttStatusTopic", TEMPread);
    mqttClient.publish(mqttStatusTopic, state == HIGH ? "OFF" : "ON", true, 1);
    // mqttClient.publish(mqttActionTopic, state == HIGH ? "OFF" : "ON", true, 1);
...
void handleRoot()
{
  // -- Let IotWebConf test and handle captive 0al requests.
  if (iotWebConf.handleCaptivePortal())
  {
    // -- Captive portal request were already served.
    return;
  }
  //serverdan değer döndürür
  ...
  if (server.hasArg("tempoda"))
  {
    String temp = server.arg("tempoda");
    weboda = temp.toInt();
    //  mqttoda=weboda;
    Serial.print(" webden gelen tempoda : ");
    Serial.println(weboda);
    applyAction(millis());
  }
  if (server.hasArg("tempyatak"))
  {
    String temp = server.arg("tempyatak");
    webyatak = temp.toInt();
    // mqttyatak=webyatak;
    Serial.print(" webden gelen  tempyatak : ");
    Serial.println(webyatak);
    applyAction(millis());
  }
  if (server.hasArg("tempcocuk"))
  {
    String temp = server.arg("tempcocuk");
    webcocuk = temp.toInt();
    // mqttcocuk=webcocuk;
    Serial.print(" webden gelen tempcocuk : ");
    Serial.println(webcocuk);
    applyAction(millis());
  }
  ...
  s += "<div><a href='config'>Kurulum Ayarlaris</a></div>";
  ...
  //Serial.println("webffffff: " + server.arg());
...
/*void handleoODA() {                          // If a POST request is made to URI /LED
  server.send(200, "text/html", "<form action=\"/handleODA\" method=\"POST\"><input type=\"static\" value=\" \"></form>");


  String veri = "gelen veri yok\n\n";
  ...
  message += server.args();
  message += "\n";
  // konsoldan gönder // curl -d "param1=value1&param2=value2" -X POST http://192.168.1.33/handleTEMP
  for (uint8_t i = 0; i < server.args(); i++) {  // gelen tüm verileri sıralar
  ...
  Serial.print("veri.arg ODA  : ");
  Serial.print(veri);
   Serial.println("ODA post alınan veri message  : ");    // Change the state of the LED
  Serial.println( message);
  // Serial.println("bitti  : 2 ");
  server.sendHeader("Location","/");        // Add a header to respond with a new location for the browser to go to the home page again
  //server.send(404, "text/plain", message);
  // Serial.println("bitti  :3 ");
  server.send(303);                         // Send it back to the browser with an HTTP status 303 (See Other) to redirect
  // Serial.println("bitti  :4 ");
  }
  void handleYATAK() {                          // If a POST request is made to URI /LED
  server.send(200, "text/html", "<form action=\"/handleYATAK\" method=\"POST\"><input type=\"static\" value=\" \"></form>");
  ...
  Serial.print("veri.arg YATAK  : ");
  Serial.print(veri);
   Serial.println("YATAK post alınan veri message  : ");    // Change the state of the LED
  Serial.println( message);
  // Serial.println("bitti  : 2 ");
  server.sendHeader("Location","/");        // Add a header to respond with a new location for the browser to go to the home page again
  //server.send(404, "text/plain", message);
  ...
  void handleCOCUK() {                          // If a POST request is made to URI /LED
  server.send(200, "text/html", "<form action=\"/handleCOCUK\" method=\"POST\"><input type=\"static\" value=\" \"></form>");
  ...
  Serial.print("veri.arg COCUK  : ");
  Serial.print(veri);
   Serial.println("COCUK post alınan veri message  : ");    // Change the state of the LED
  Serial.println( message);
  // Serial.println("bitti  : 2 ");
  server.sendHeader("Location","/");        // Add a header to respond with a new location for the browser to go to the home page again
  //server.send(404, "text/plain", message);
...
bool formValidator(iotwebconf::WebRequestWrapper* webRequestWrapper)
{
  ...
  if (l < 3)
  {
    mqttServerParam.errorMessage = "Server adresi girin!(broker.hivemq.com)";
    valid = false;
  }
