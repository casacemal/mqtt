...
bool IotWebConf::loadConfig()
{
  ...
  {
    IOTWEBCONF_DEBUG_LINE(F("HATALI  CONFİG version.  defaults AYARLAR UYGULANIYOR."));
...
//    page += _customHeadElement;
    ...
    {
      page += F("Devam etmek için kendi wifi agına baglanmalısın. Return "
                "to <a href=''>configuration page</a>.");
    }
  ...
  switch (newState)
  {
    case ApMode:
    {
      ...
      if (mustUseDefaultPassword())
      {
        ...
        {
			Serial.println("");
          Serial.println("                       AP parolası yapılandırmada ayarlanmadı");
		  Serial.println("");
		  Serial.println("");
        }
#endif
