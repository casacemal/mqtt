...
void setup() 
{
  ...
  server.on("/", handleRoot);
   server.on("/handleTEMP",HTTP_POST, handleTEMP);
  server.on("/config", []{ iotWebConf.handleConfig(); });
  server.onNotFound([](){ iotWebConf.handleNotFound(); });

  //webden alınan bilgiyi mqtt olarak gönderir.///devices/testThing/status
...
void handleTEMP() {                          // If a POST request is made to URI /LED
  server.send(200, "text/html", "<form action=\"/handleTEMP\" method=\"POST\"><input type=\"static\" value=\" \"></form>");

  Serial.print("post alınan veri  : ");    // Change the state of the LED
...
Serial.print("veri.arg  : ");
Serial.print(veri);
   Serial.println("post alınan veri message  : ");    // Change the state of the LED
